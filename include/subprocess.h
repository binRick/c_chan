...
subprocess_weak int
subprocess_create_ex(const char *const command_line[], int options, const char *const environment[], struct subprocess_s *const out_process);
...
subprocess_read_stdout(struct subprocess_s *const process, char *const buffer, unsigned size);
...
/// conjuction with this method.
subprocess_weak unsigned
subprocess_read_stderr(struct subprocess_s *const process, char *const buffer, unsigned size);
...
#pragma warning(pop)
  ...
  *wr = CreateFileA(name, genericWrite, SUBPROCESS_NULL,
  ...
  return(0);
} // subprocess_create_named_pipe_helper
#endif
